      ...
      for(let command of commands) {
        ...
        const options = {
          ...
          options: {
            ...
            isModerator: command?.options?.isModerator ? command?.options?.isModerator : false,
        ...
        if(command?.command instanceof RegExp) {
          CommandsRegex.set(options?.name, options)
        } else {
          Commands.set(command?.command[0], options);
        }
      }
    });
    return { command: Commands, regex: CommandsRegex };
  } catch (e) {
