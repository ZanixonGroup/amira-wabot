    ...
    try {
      ...
      return await new Promise(async (resolve, reject) => {
        ...
        axios.post('https://api.imggen.ai/guest-upload', form, { headers: form.getHeaders() }).then(async tmp => {
          ...
          // request
          axios.post("https://api.imggen.ai/guest-remove-text", payload).then(async res => {
            const data = res.data;
            if(!data?.removed_text_image) return reject("failed remove text image");
            resolve({
              status: true,
              image: "https://api.imggen.ai" + data.removed_text_image
            });
          }).catch(reject);
        }).catch(reject);
      })
