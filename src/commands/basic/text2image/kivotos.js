  ...
  code: async({ client, m, remote, text, plugins, Func, MessageBuilder, facemoji, logs, alertMessage }) => {
    try {
      ...
      if(!text) return m.reply(`Berikan prompt gambar nya kak, amira akan berusaha membuatkan gambar nya ${facemoji.joy}\n` +
        ...
        `\n` +
        `*Contoh:*\n` +
        `> .kivotos ayase yuuka, beautiful, looking at viewer --ar 5:12`
      , message);
      ...
      const kvs = await plugins.kivotosXL({
        prompt,
        width: scale.w,
        height: scale.h,
        aspect_ratio: scale.reso
      });
      if(!kvs.status) return m.reply(`Maaf kak, amira gagal untuk membuatkan gambarnya ${facemoji.sad}`, message);
      const image = kvs.data.images[0];
      const meta = kvs.data.metadata;
