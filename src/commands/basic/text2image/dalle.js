export default [{
  ...
  code: async({ client, m, remote, text, plugins, MessageBuilder, facemoji, logs, alertMessage }) => {
    try {
      ...
      const dalle = await plugins.dalle(text);
      if(!dalle.status) return m.reply(`Maaf kak, amira gagal untuk membuatkan gambarnya ${facemoji.sad}`, message);
      /* if(filter.data.isNsfw) {
      const filter = await plugins.nsfwFilter(dalle.data)
...
*Prompt:*
> ${text}`;
      client.sendMedia(remote, dalle.data, m, {
        caption,
        mimetype: "image/png",
        filename: "dalle.png",
        ...message
