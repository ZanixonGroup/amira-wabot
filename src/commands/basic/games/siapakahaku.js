export default [{
  ...
  code: async({ client, m, remote, sender, isCommand, fs, dirname, path, MessageCollector, MessageBuilder, facemoji, userId, hint }) => {
    try {
      ...
      let games = JSON.parse(await fs.readFileSync(path.join(dirname(import.meta.url), "../../../../assets/games/siapakahaku.json"), "utf8"));
...
> Reward berhasil ditambahkan ke akun kakak ${facemoji.happy}`, message);
          await client.knex("users").where({ user_id: sender }).update("ticket", ticket + 1);
          col.collected()
        } else {
