export default [{
  ...
  code: async({ client, m, text, plugins, MessageBuilder, facemoji, logs, alertMessage }) => {
    try {
      ...
      if(!text) return m.reply(`Berikan pertanyaan nya kak, gemma akan berusaha menjawabnya ${facemoji.joy}`, message);
      const chat = [
        {
          role: "user",
      ...
      const gemma = await plugins.gemma(chat);
      const content = gemma?.data?.choices[0]?.message?.content;
